using S7.Net;
using Station.Hardware.PLC;

namespace Station.Hardware.Services;
...
public class PLCReliableService : IDisposable
{
    ...
    public List<PointBase> Points { get; set; }

    public event Action<string> LogReceived;
    public event Action<bool, bool> DataReceived;
    ...
    private async Task 定时读取()
    {
        try
        {
            ...
            // 读DB块示例
            await _plc.BatchRead(pLCConfig.DBNumber, Points) ;
        }
        catch (Exception ex)
        {
            LogReceived?.Invoke($"PLC读取异常: {ex.Message}");
